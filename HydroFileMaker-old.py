...
class HydroFileMaker:
	...
	def run(self):
		...
		#Load ther vector layers into ther combo box
		...
		if result:
			...
			#itterate thoughe ther raster layers peforming grid statistics on each
			selectedLayerIndex = self.dlg.comboBox.currentIndex()
			vectorLayer = layers[selectedLayerIndex]
			#Delete to cell river code
			fList = list()
			fList.append(0)
				...
				if self.dlg.listWidget.item(x).isSelected():
					for name, search_layer in QgsMapLayerRegistry.instance().mapLayers().iteritems():
						if search_layer.name() == self.dlg.listWidget.item(x).text():
							...
							if check:
								output = processing.runalg('saga:gridstatisticsforpolygons', rasterLayer.source(), vectorLayer, False, False, False, False, False, True, False, False, 0, None)
								tempLayer = self.iface.addVectorLayer(output['RESULT'], "tempLayer", "ogr")
								check = 0
							else:
								output = processing.runalg('saga:gridstatisticsforpolygons', rasterLayer.source(), tempLayer, False, False, False, False, False, True, False, False, 0, None)
								QgsMapLayerRegistry.instance().removeMapLayer( tempLayer.id() )
								tempLayer = self.iface.addVectorLayer(output['RESULT'], "tempLayer", "ogr")
			#write the vecto file
			#QgsVectorFileWriter.writeAsVectorFormat(tempLayer, filename, "utf-8", None, "ESRI Shapefile")
			...
			#Get ther fields form ther layer
			fields = tempLayer.pendingFields()
			...
			for flow in fieldNames:
				...
				flow2 =  flow.partition("[")[0]
				flowList[n] = float(flow2[1:])
				n = n + 1
			flowList = set(flowList)
